...
void* setFirst(void * threadArgs) {
    ThreadArgs* args = (ThreadArgs *) threadArgs;

    args->person = args->queue[args->i];
    pthread_exit(NULL);
}

void* setOut(void* out) {
    ...
    while (aux1 < lenFQ || aux2 < lenSQ) {
        ...
        if (first.direction == 0) {
            ...
            if ((aux1 < lenFQ && (firstQueue[aux1].time <= out) || (firstQueue[aux1].time > out && firstQueue[aux1].time < secondQueue[aux2].time)) || aux2 == lenSQ) {
                ...
                for (int i = 0; i < lenFQ; i++) args.queue[i] = firstQueue[i];
                
                args.i = aux1;

                pthread_create(&threadID, NULL, setFirst, (void *) &args);
